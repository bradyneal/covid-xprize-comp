...
min_pred = min(lstm_predictions_df['PredictedDailyTotalDeaths'].min(), lgbm_predictions_df['PredictedDailyTotalDeaths'].min())
max_pred = min(lstm_predictions_df['PredictedDailyTotalDeaths'].max(), lgbm_predictions_df['PredictedDailyTotalDeaths'].max())
...
plt.xlabel("Day")
plt.ylabel("Predicted Daily Total Deaths")
p, = plt.plot(x, y, linewidth=2, color='blue')
plt.axis([0, num_days, min_pred, max_pred])
...
def get_pred(alpha, lstm_predictions_df, lgbm_predictions_df):
    lstm_pred = lstm_predictions_df['PredictedDailyTotalDeaths']
    lgbm_pred = lgbm_predictions_df['PredictedDailyTotalDeaths']
...
def val_update(val):
    ...
    ensemble_data['PredictedDailyTotalDeaths'] = get_pred(slider1.val, lstm_predictions_df, lgbm_predictions_df)
    ...
    p.set_ydata(ensemble_data['PredictedDailyTotalDeaths'])
    print(slider1.val)
    print(ensemble_data)
    plt.draw()
